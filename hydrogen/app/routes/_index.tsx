...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  const [
    ...
    // NOTE: Judge.me reviews/stats are NOT fetched here — they're deferred below (reviewsDeferred)
    // so they never block TTFB. They must stay out of this array: the destructuring above has no
    // slots for them, so leaving them in shifted every later result by two (homeLayoutRes received
    // the reviews response), which emptied `section_order` and made hidden home sections appear.
...
export default function Home() {
  ...
  const sections: Record<string, React.ReactNode> = {
    ...
    reels: <Suspense key="reels" fallback={null}><ReelsCarousel reels={reels} label={reelsLabel} heading={reelsHeading} /></Suspense>,
