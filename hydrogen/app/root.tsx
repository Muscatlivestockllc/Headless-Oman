...
// Supports up to 4 free-product slots per rule (free_variant, free_variant_2..4), each with its own
// quantity (free_variant_qty, free_variant_2_qty, …). A single rule can therefore hand out several
// different free products in one order. Each slot is expanded into its own engine rule sharing the
// rule's match criteria, so the existing per-variant sync logic adds every gift independently.
function parseFreeGiftRules(nodes: any[]) {
  return (nodes ?? []).flatMap((n: any) => {
    ...
    // Up to 4 free-product slots per rule, each with its own quantity.
    const slots = [
    ...
    // One engine rule per free variant, so every gift in the rule gets added.
